...
const ExportButtons = ({ onExport, walletAddress, dateRange }) => {
  ...
  const handlePaymentComplete = async (paymentRequestId) => {
    setIsExporting(true)
    
    try {
      // Now that payment is confirmed, proceed with the actual export
      await onExport(currentExportType, paymentRequestId)
      
      toast.success(`${currentExportType.toUpperCase()} export completed successfully!`, {
        icon: '📄',
        duration: 4000,
      })
      ...
      toast.error(`Failed to export ${currentExportType.toUpperCase()}. Please try again.`)
    } finally {
